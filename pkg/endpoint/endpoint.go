...
func (e *Endpoint) GetPolicyModel() *models.EndpointPolicyStatus {
	...
	for policyMapKey := range e.realizedMapState {
		...
		default:
			log.WithField(logfields.TrafficDirection, policymap.TrafficDirection(policyMapKey.TrafficDirection)).Errorf("unexpected traffic direction present in realized PolicyMap state for endpoint")
		}
	}
	...
	for policyMapKey := range e.desiredMapState {
		...
		default:
			log.WithField(logfields.TrafficDirection, policymap.TrafficDirection(policyMapKey.TrafficDirection)).Errorf("unexpected traffic direction present in desired PolicyMap state for endpoint")
		}
	}
