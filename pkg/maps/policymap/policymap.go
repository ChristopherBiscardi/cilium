...
// GetPort returns the port in key.
...
// protocol `proto`. It is assumed that `dport` is in network byte-order.
func (pm *PolicyMap) Allow(id uint32, dport uint16, proto u8proto.U8proto, trafficDirection TrafficDirection) error {
	key := PolicyKey{Identity: id, DestPort: dport, Nexthdr: uint8(proto), TrafficDirection: trafficDirection.Uint8()}
	entry := PolicyEntry{}
...
// `dport`over protocol `proto`. It is assumed that `dport` is in network byte-order.
...
// over protocol `proto`. It is assumed that `dport` is in network byte-order.
